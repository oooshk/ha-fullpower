"""
Live MQTT client — connects to Kiso's broker exactly as the app does:

    broker   : tcp://appglobal.kisoiot.com:3010   (plain TCP)
    username : <accessToken from REST login>
    password : ""
    clientId : "androidId" + 10 random digits
    subscribe: "/#"

Message shape (verified from capture):
    {"content": "<escaped JSON string>", "mac": "...", "deviceType": "...",
     "msgId": N, "msgType": "P013"}
The real OCPP data (ChargePointStatus, V_L1, chargingPower, ...) lives INSIDE the
`content` string, so we unwrap it, filter to our device, and merge into the
coordinator for instant updates. Every raw message is also written to the
capture log for local-control work.
"""
...
class FullPowerMqtt:
    ...
    def _on_message(self, client, userdata, msg):
        try:
            ...
            text = ""
        # Capture every raw message (executor; add_job is thread-safe).
        self._hass.add_job(
            capture.append, self._hass.config.config_dir, self._mac, "mqtt",
            text, msg.topic,
        )
